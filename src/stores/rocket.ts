...
export const useRocketStore = defineStore('rocket', () => {
  ...
  async function fetchRockets() {
    ...
    error.value = null
    try {
      const res = await fetch(`${API_BASE_URL}/?manufacturer__name=SpaceX&mode=detailed&limit=20`)
      if (!res.ok) {
      ...
      error.value = err instanceof Error ? err.message : 'Failed to fetch rockets from API'
    } finally {
  ...
  async function fetchRocketById(id: string | number) {
    ...
    const localFound = localRockets.value.find(r => String(r.id) === String(id))
    if (localFound) {
    ...
    try {
      ...
      detailError.value = err instanceof Error ? err.message : 'Error occurred while loading rocket details'
    } finally {
      isDetailLoading.value = false
    }
  }
